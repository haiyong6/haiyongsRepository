				...
				<c:forEach items="${manfLetter.objList}" var="manf">
					<tr>
						<td class="selectorTypeTd" style="padding-top: 0px;">
						      <input type="hidden" value="${manf.manfId }"/>${manf.manfName}
						</td>
						<td class="selectorContentTd">
							<div class="row-fluid">
								<c:forEach items="${manf.subModelList}" var="subModel">
									<label style="width:120px;padding:0px;margin-left:20px;" class="span3 checkbox subModelIdManfLabel" letter="${subModel.letter}" ><input  name="subModelIdInputByManf" type="${inputType == 1?'checkbox':'radio'}" class="subModelIdInput" pooAttributeId="${subModel.pooAttributeId}" value="${subModel.subModelId}">${subModel.subModelName}</label>
								</c:forEach>
							</div>
						</td>
					</tr>
